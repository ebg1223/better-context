...
btca --help`;
	...
	const CHAT_CMD = `btca chat -t effect`;
...
<section class="flex flex-col gap-10">
	<div class="flex flex-col gap-4">
		<div class="inline-flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-400">
			...
			>
			<span class="hidden sm:inline">Ask a Question, Search the Actual Codebase, Get a Real Answer.</span>
		</div>

		<h1 class="text-balance text-4xl font-semibold tracking-tight text-neutral-950 dark:text-neutral-50 sm:text-5xl">
			Up to Date Info About any Technology
		</h1>

		<p class="max-w-2xl text-pretty text-base leading-relaxed text-neutral-700 dark:text-neutral-300 sm:text-lg">
			<code class="rounded bg-neutral-900/5 px-1.5 py-1 text-sm dark:bg-white/10">btca</code>
			is a CLI for asking questions about libraries/frameworks by cloning their repos locally and searching the source directly
		</p>
	...
	<section id="install" class="scroll-mt-28">
		<h2 class="text-xl font-semibold tracking-tight text-neutral-950 dark:text-neutral-50">Install</h2>
		<p class="mt-2 max-w-2xl text-sm leading-relaxed text-neutral-700 dark:text-neutral-300">
			Install globally with Bun, then run <code class="rounded bg-neutral-900/5 px-1.5 py-1 text-xs dark:bg-white/10">btca --help</code>.
		</p>
		<div
		...
		>
			...
			<pre
				class="max-w-full overflow-x-auto p-4 pr-14 text-sm leading-relaxed text-neutral-900 dark:text-neutral-50"
			><code>{INSTALL_CMD}</code></pre>
		</div>
	</section>

	<section id="commands" class="scroll-mt-28">
		<h2 class="text-xl font-semibold tracking-tight text-neutral-950 dark:text-neutral-50">Quick commands</h2>
		<p class="mt-2 max-w-2xl text-sm leading-relaxed text-neutral-700 dark:text-neutral-300">
		...
		<div class="mt-4 grid gap-4 md:grid-cols-2">
			...
			>
				<div class="text-sm font-semibold text-neutral-900 dark:text-neutral-50">Ask a question</div>
				<div
			...
			>
				...
				>
					...
					<pre
						class="max-w-full overflow-x-auto p-4 pr-14 text-sm leading-relaxed text-neutral-900 dark:text-neutral-50"
					><code>{CHAT_CMD}</code></pre>
				</div>
			</div>
			<div
				class="min-w-0 rounded-2xl border border-neutral-200 bg-white/70 p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900/30"
			>
				<div class="text-sm font-semibold text-neutral-900 dark:text-neutral-50">Run as a server</div>
				<div
					class="relative mt-3 min-w-0 overflow-hidden rounded-xl border border-neutral-200 bg-white/70 dark:border-neutral-800 dark:bg-neutral-950/40"
				>
					<div class="absolute right-3 top-3">
					...
					<pre
						class="max-w-full overflow-x-auto p-4 pr-14 text-sm leading-relaxed text-neutral-900 dark:text-neutral-50"
					><code>{SERVE_CMD}</code></pre>
				</div>
				<div class="mt-3 text-sm text-neutral-600 dark:text-neutral-400">
					POST <code class="rounded bg-neutral-900/5 px-1 py-0.5 text-xs dark:bg-white/10">/question</code> with
			...
			<div
				class="min-w-0 rounded-2xl border border-neutral-200 bg-white/70 p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900/30"
			>
				<div class="text-sm font-semibold text-neutral-900 dark:text-neutral-50">Keep an OpenCode instance running</div>
				<div
					class="relative mt-3 min-w-0 overflow-hidden rounded-xl border border-neutral-200 bg-white/70 dark:border-neutral-800 dark:bg-neutral-950/40"
				>
					<div class="absolute right-3 top-3">
	...
	<section id="config" class="scroll-mt-28">
		<h2 class="text-xl font-semibold tracking-tight text-neutral-950 dark:text-neutral-50">Config</h2>
		<p class="mt-2 max-w-2xl text-sm leading-relaxed text-neutral-700 dark:text-neutral-300">
			On first run, <code class="rounded bg-neutral-900/5 px-1.5 py-1 text-xs dark:bg-white/10">btca</code> creates a default
			config at <code class="rounded bg-neutral-900/5 px-1.5 py-1 text-xs dark:bg-white/10">~/.config/btca/btca.json</code>.
